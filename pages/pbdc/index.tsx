import { Alert, Box, Button, ButtonGroup, Divider, Fab, Grid, IconButton, InputAdornment, List, Snackbar, TextField, Typography } from "@mui/material";
...
const Pbdc = () => {
    ...
    const handleSave = () => {
        onSave(pbdcDraft.id,pbdcDraft.nopb,pbdcDraft.cab,pbdcDraft.tipe,pbdcDraft.dc,pbdcDraft.tgl,pbdcDraft.nilai,pbdcDraft.status,pbdcDraft.details);
    }
        ...
        <>
            <Box sx={{
                ...
                <Grid xs={12} sx={{
                    ...
                    <Typography fontWeight={'bold'}>List Penerimaan Barang DC</Typography>
                </Grid>
                <Grid xs={12} sx={{ paddingTop: "10px", alignContent: "center" }}>
                    <TextField
                        label="Pencarian"
                        id="search-filter"
            ...
            <Box sx={{
                ...
                <Grid xs={12} sx={{
                    ...
                    <List sx={{ width: '100%', height: "800px", bgcolor: 'background.paper', overflow: "scroll" }}>
                        {pbdcs?.map((value) => {
                            return (<><LsiListItem key={value.nopb} id={value.nopb} title={`No PB: ${value.nopb} - ${value.tgl}`}
                            subtitle={`Tipe: ${value.tipe} | DC: ${value.dc} | Nilai: ${value.nilai}`} onDelete={(id: number): void => {
                                alert('deleting')
